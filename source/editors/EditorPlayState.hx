...
class EditorPlayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (notetype in noteTypeMap.keys()) {
			...
			if (notetype == 'ES Halo Note')
				luaToLoad = Paths.modFolders('custom_notetypes/' + 'Halo Note' + '.lua');
			if (notetype == 'Extra Strum Note')
				luaToLoad = Paths.modFolders('custom_notetypes/' + '.lua');

			if(sys.FileSystem.exists(luaToLoad)) {
				trace('EXISTS!!' + luaToLoad);
					...
					if(daStrumTime >= startPos) {
						...
						if(floorSus > 0) {
							for (susNote in 0...floorSus+1)
							{
								...
								var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(PlayState.SONG.speed, 2)), daNoteData, oldNote, true, true);
