...
class NoteOffsetState extends MusicBeatState
{
	...
	function reloadTexts()
	{
		for (i in 0...dumbTexts.length)
		{
			if (!CoolUtil.isPTBR()) {
				...
				switch(i)
				{
					case 0: dumbTexts.members[i].text = 'Deslocamento do Julgamento:';
					case 1: dumbTexts.members[i].text = '[' + ClientPrefs.comboOffset[0] + ', ' + ClientPrefs.comboOffset[1] + ']';
					case 2: dumbTexts.members[i].text = 'Deslocamento dos Números:';
					case 3: dumbTexts.members[i].text = '[' + ClientPrefs.comboOffset[2] + ', ' + ClientPrefs.comboOffset[3] + ']';
				}
			}
		}
	...
	function updateNoteDelay()
	{
		...
		if (!CoolUtil.isPTBR())
			...
			timeTxt.text = 'Deslocamento Atual: ' + Math.floor(barPercent) + ' ms';
	}
	...
	function updateMode()
	{
		...
		{
			if(onComboMenu)
				changeModeText.text = '< Deslocamento do Combo (Pressione \"Confirmar\" para Mudar) >';
			else
				changeModeText.text = '< Atraso da Nota/Batida (Pressione \"Confirmar\" para Mudar) >';		
		}
