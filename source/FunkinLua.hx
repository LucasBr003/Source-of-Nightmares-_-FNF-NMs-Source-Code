...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('fileCreation', ClientPrefs.files);
		set('scaryFiles', ClientPrefs.scary);
		set('rgbIntensity', ClientPrefs.rgbIntensity);
		set('shakeIntensity', ClientPrefs.shakeIntensity);
		set('animIcons', ClientPrefs.animIcons);
		set('babyMode', ClientPrefs.baby);
		set('portuguese', ClientPrefs.ptbr);

		set('showScore', ClientPrefs.showScore);
		set('showMisses', ClientPrefs.showMisses);
		set('showAcc', ClientPrefs.showAcc);
		set('showHealth', ClientPrefs.showHealth);
		set('showJudgements', ClientPrefs.showJudgements);

		// Window Stuff
		set('defaultWindowWidth', 1280);
		set('defaultWindowHeight', 720);
		...
		// SKIP TIME YAAAAY
		Lua_helper.add_callback(lua, "skipTime", function(time:Float) {
			PlayState.ignoreEvents = true;
			PlayState.instance.clearNotesBefore(time);
			PlayState.instance.setSongTime(time);
			PlayState.ignoreEvents = false;
		});

		// Window Stuff
		Lua_helper.add_callback(lua, "window_move", function(toX:Int, toY:Int) { // Screen movement on LUA!!!
			Lib.application.window.move(toX, toY);
		});
		Lua_helper.add_callback(lua, "window_alert", function(message:String, title:String) { // Error on LUA!!!
			Lib.application.window.alert(message, title);
		});
		Lua_helper.add_callback(lua, "window_close", function() { // Crash on LUA!!!
			Lib.application.window.close();
		});
		Lua_helper.add_callback(lua, "window_changeSize", function(?width:Int = 1280, ?height:Int = 720) { // WINDOW RESIZE on LUA!!!!!!!!!
			Lib.application.window.resize(width, height);
		});
		Lua_helper.add_callback(lua, "window_setResizable", function(resizable:Bool) { // Ok enough
			Lib.application.window.resizable = resizable;
		});
		Lua_helper.add_callback(lua, "window_fullScreen", function(fullscreen:Bool) {
			Lib.application.window.fullscreen = fullscreen;
		});
		Lua_helper.add_callback(lua, "window_freeze", function(time:Float) {
			Sys.sleep(time);
		});

		Lua_helper.add_callback(lua, "window_screenCenter", function(aixis:String) {
			switch(aixis)
			{
				case 'X':
					Lib.application.window.move(Math.floor(1280 / 4), Lib.application.window.y);
				case 'Y':
					Lib.application.window.move(Lib.application.window.x, Math.floor(720 / 4));
				
				default:
					Lib.application.window.move(Math.floor(1280 / 4), Math.floor(720 / 4));
			}
		});
		...
		Lua_helper.add_callback(lua, "getPropertyFromGroup", function(obj:String, index:Dynamic, variable:Dynamic) {
			if(Std.isOfType(Reflect.getProperty(getInstance(), obj), FlxTypedGroup)) {
				return getGroupStuff(Reflect.getProperty(getInstance(), obj).members[Std.parseInt(index)], variable);
			}

			var leArray:Dynamic = Reflect.getProperty(getInstance(), obj)[Std.parseInt(index)];
			if(leArray != null) {
			...
			luaTrace("Object #" + Std.parseInt(index) + " from group: " + obj + " doesn't exist!");
			return null;
		});
		...
		// gay ass tweens
		Lua_helper.add_callback(lua, "doTweenX", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, ?easeEffectSpeed:Float = 1) {
		...
		Lua_helper.add_callback(lua, "doTweenY", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, ?easeEffectSpeed:Float = 1) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
		...
		Lua_helper.add_callback(lua, "doTweenAngle", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, ?easeEffectSpeed:Float = 1) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
		...
		Lua_helper.add_callback(lua, "doTweenAlpha", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, ?easeEffectSpeed:Float = 1) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
		...
		Lua_helper.add_callback(lua, "doTweenZoom", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, ?easeEffectSpeed:Float = 1) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
		...
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String, ?easeEffectSpeed:Float = 1) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
		...
		Lua_helper.add_callback(lua, "setTotalPlayed", function(value:Int = 0) {
			PlayState.instance.totalPlayed = value;
			PlayState.instance.RecalculateRating();
		});
		Lua_helper.add_callback(lua, "setTotalNotesHit", function(value:Int = 0) {
			PlayState.instance.totalNotesHit = value;
			PlayState.instance.RecalculateRating();
		});
		Lua_helper.add_callback(lua, "setRating", function(value:Int = 0) {
		...
		Lua_helper.add_callback(lua, "keyPressed", function(name:String) {
			...
			switch(name) {
				...
				case 'dodge': key = PlayState.instance.getControl('DODGEB');
				case 'erase': key = PlayState.instance.getControl('ERASE');
		...
		Lua_helper.add_callback(lua, "getKeybind", function(key:String) {
			return ClientPrefs.copyKey(ClientPrefs.keyBinds.get(key));
		});
